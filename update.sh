...
remove_unwanted_packages() {
    local luci_packages=(
        ...
        "luci-app-msd_lite"
        "luci-app-nikki" "luci-app-tailscale" "luci-app-advancedplus" "luci-app-iperf3-server" "luci-app-upnp"
    )
    local packages_net=(
...
src/gz openwrt_telephony https://dl.openwrt.ai/packages-24.10/aarch64_cortex-a53/kiddin9/
EOF
...
set_build_signature() {
    ...
    if [ -d "$(dirname "$file")" ] && [ -f $file ]; then
        sed -i "s/(\(luciversion || ''\))/(\1) + (' \/ build by Logan')/g" "$file"
    fi
}
